...
export function TaskCard({ messageId }: Props) {
  ...
  const handleCompleteMyPart = () => {
    ...
    if (checklistIncomplete) {
      ...
      updateAssigneeStatus.mutate({ assigneeId: myAssignment!.id, status: "completed", taskInstanceId: task.id });
    }
  };

  const confirmWithObservation = () => {
    if (!observation.trim()) return;
    if (observationDialog?.type === "complete") {
      updateStatus.mutate({ taskId: task.id, status: "completed", comment: `⚠️ Checklist incompleto (${checkedCount}/${checklistTotal}): ${observation.trim()}` });
    } else if (observationDialog?.type === "completePart" && observationDialog.assigneeId) {
      updateAssigneeStatus.mutate({ assigneeId: observationDialog.assigneeId, status: "completed", taskInstanceId: task.id });
    }
  ...
  return (
    <>
      ...
      {/* Observation dialog for incomplete checklist */}
      <Dialog open={!!observationDialog} onOpenChange={(o) => !o && setObservationDialog(null)}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              O checklist está com <strong>{checkedCount}/{checklistTotal}</strong> itens preenchidos.
              Informe o motivo para concluir sem completar todos os itens.
            </p>
            <Textarea
              value={observation}
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setObservationDialog(null)}>Cancelar</Button>
            <Button
              onClick={confirmWithObservation}
